...
/*   Updated: 2020/08/16 15:04:34 by nforay           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	update_env_pwd(t_list *env, t_shell *shell, t_list *command)
{
	char	buf[PATH_MAX];
	char	*tmp;
	char	*ptr;
	...
	if (!(shell->cwd = getcwd(buf, PATH_MAX)))
...
static int	changedir_home(t_list *env, t_shell *shell, t_list *command)
{
	...
	{
		update_env_pwd(env, shell, command);
